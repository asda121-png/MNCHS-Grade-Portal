document.addEventListener('DOMContentLoaded', function () {
    ...
    if (addModal) {
        ...
        const teacherAdviserClassSelect = document.getElementById('teacherAdviserClass');
        ...
        const loadAdviserClasses = async () => {
            try {
                const response = await fetch('../../server/api/teachers.php?action=get_classes');
        ...
        openAddModal = (mode = 'add', data = null) => {
            if (addTeacherForm) addTeacherForm.reset();
            if (modalTitle) modalTitle.textContent = mode === 'edit' ? 'Edit Teacher' : defaultModalTitle;
            if (saveButton) saveButton.textContent = mode === 'edit' ? 'Update Teacher' : defaultSaveText;
            if (addTeacherForm) {
            ...
            if (data) {
                ...
                const initialSchoolLevel = data.schoolLevel || deriveSchoolLevel(gradeLevelValue);
        ...
        if (addTeacherForm) {
            addTeacherForm.addEventListener('submit', async (e) => {
                ...
                const schoolLevel = document.getElementById('teacherSchoolLevel')?.value || '';
                const gradeLevel = document.getElementById('teacherGradeLevel')?.value || '';
                const role = document.getElementById('teacherStatus')?.value || 'subject-teacher';
                const adviserClass = document.getElementById('teacherAdviserClass')?.value || '';
                const nameParts = [firstName, middleName, lastName, suffix];
                const fullName = nameParts.filter(part => part).join(' ');
                const actionText = addTeacherForm.dataset.mode === 'edit' ? 'Updating' : 'Saving';
                const identifier = addTeacherForm.dataset.teacherId ? `ID: ${addTeacherForm.dataset.teacherId}\n` : '';
                const roleDisplay = role === 'adviser' && adviserClass ? `Adviser to ${adviserClass}` : role;
                ...
                if (addTeacherForm.dataset.mode === 'edit' && teacherId) {
                    try {
                        const response = await fetch('../../server/api/teachers.php?action=update_teacher', {
                            method: 'POST',
    ...
    if (viewModal) {
        ...
        viewModal.querySelectorAll('[data-close="view"]').forEach(button => {
            button.addEventListener('click', () => viewModal.classList.remove('show'));
        });

        viewModal.showDetails = (data) => {
            if (!data) return;
            if (viewFields.employeeId) viewFields.employeeId.textContent = displayText(data.employeeId || data.id);
            ...
            if (viewFields.schoolLevel) viewFields.schoolLevel.textContent = displayText(data.schoolLevel || deriveSchoolLevel(data.gradeLevel));
            if (viewFields.department) viewFields.department.textContent = displayText(data.department);
            if (viewFields.specialization) viewFields.specialization.textContent = displayText(data.specialization);
            if (viewFields.gradeLevel) viewFields.gradeLevel.textContent = displayText(data.gradeLevelLabel || data.gradeLevel);
            if (viewFields.adviserClass) viewFields.adviserClass.textContent = displayText(data.adviserClass || 'Not an Adviser');
            if (viewFields.hireDate) viewFields.hireDate.textContent = displayText(data.hireDate, formatDate);
    ...
    const applyFilters = () => {
        ...
        for (let i = 0; i < tableRows.length; i++) {
            ...
            const employeeId = row.dataset.teacherEmployeeId || row.cells[0]?.textContent || '';
            const name = row.dataset.teacherName || row.cells[1]?.textContent || '';
            const gradeLevelLabel = row.dataset.teacherGradeLevelReadable || row.dataset.teacherGradeLevel || '';
            const sectionsLabel = row.dataset.teacherSectionsReadable || '';
            const schoolLevelLabel = row.dataset.teacherSchoolLevel || '';
            const searchMatch = employeeId.toLowerCase().includes(searchText)
            ...
            const rowGrade = (row.dataset.teacherGradeLevel || '').toLowerCase();
            const rowGradeReadable = (row.dataset.teacherGradeLevelReadable || '').toLowerCase();
            const gradeMatch = !gradeValue || rowGrade === gradeValue || rowGradeReadable.includes(gradeValue);
    ...
    teacherTableBody.addEventListener('click', (e) => {
        ...
        const gradeLevelLabel = dataset.teacherGradeLevelReadable || (gradeLevel ? `Grade ${gradeLevel}` : '');
        const sectionsReadable = dataset.teacherSectionsReadable || '';
        const sectionsRaw = dataset.teacherSections || '';
        const schoolLevel = dataset.teacherSchoolLevel || deriveSchoolLevel(gradeLevel);

        const teacherData = {
            id: dataset.teacherId || '',
            employeeId: dataset.teacherEmployeeId || row.cells[0]?.textContent.trim() || '',
