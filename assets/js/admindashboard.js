...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const closeManageEventsBtn = document.getElementById('close-manage-events-btn');
    const addEventModal = document.getElementById('add-event-modal');
    const addEventForm = document.getElementById('add-event-form');
    ...
    if (addEventForm) {
        addEventForm.addEventListener('submit', (e) => {
            ...
            const isPublished = document.getElementById('add-event-published').checked;
    ...
    if (manageEventsBtn && manageEventsModal) {
        ...
        editEventForm.addEventListener('submit', (e) => {
            ...
            const isPublished = document.getElementById('edit-event-published').checked;
            ...
            .then(data => {
                ...
                if (data.success) {
                    ...
                    showNotification('Error: ' + (data.error || 'Could not update event'), 'error');
                }
            })
            .catch(error => {
                console.error('Error:', error);
        ...
        fetch('../../server/api/events.php?action=get_admin_events')
            .then(response => {
                ...
                if (!response.ok) {
                    ...
                    const message = errorMessages[response.status] || `HTTP ${response.status}: ${response.statusText}`;
    ...
    function displayEventsTable(events) {
        ...
        if (events.length === 0) {
            tableBody.innerHTML = '<tr style="text-align: center;"><td colspan="6" style="padding: 2rem; color: #999;">No events found</td></tr>';
            return;
        }
        ...
        try {
            tableBody.innerHTML = events.map(event => {
                ...
                const typeInfo = eventTypeColors[event.event_type] || eventTypeColors['other'];
                const isPublished = event.is_published == 1 || event.is_published === true;
                ...
                const title = (event.title || 'Untitled').replace(/</g, '&lt;').replace(/>/g, '&gt;');
                const eventId = event.id || '';

                return `
                    <tr style="border-bottom: 1px solid #f0f0f0;">
                        <td style="padding: 1rem 1.5rem; color: #2d3436; font-weight: 500;">${title}</td>
                        <td style="padding: 1rem 1.5rem; color: #636e72;">${formatDate(event.start)}</td>
                        <td style="padding: 1rem 1.5rem; color: #636e72;">${formatDate(event.end)}</td>
                        <td style="padding: 1rem 1.5rem;">
                            <span style="background-color: ${typeInfo.color}20; color: ${typeInfo.color}; padding: 4px 10px; border-radius: 4px; font-size: 0.85rem; font-weight: 500;">${typeInfo.label}</span>
                        </td>
    ...
    function openEditEventModal(events, eventId) {
        ...
        document.getElementById('edit-event-published').checked = event.is_published == 1;
    ...
    function deleteEvent(eventId) {
        ...
        .then(data => {
            if (data.success) {
                ...
                showNotification('Error: ' + (data.error || 'Could not delete event'), 'error');
            }
        })
        .catch(error => {
            console.error('Error:', error);
